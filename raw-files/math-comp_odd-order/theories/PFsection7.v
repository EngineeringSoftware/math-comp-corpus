...
Variant is_invDade_seqInd_sum : Prop :=
  InvDadeSeqIndSum (c := c) (u := u c) of {in A, forall x, (chi^\rho) x = \sum_(xi <- S) (c xi)^* / '[xi] * xi x}
 & '[chi^\rho] = \sum_(xi <- S) \sum_(mu <- S) u xi mu.
...
Lemma Dade_Ind1_sub_lin (nu : {additive 'CF(L) -> 'CF(G)}) zeta :
  ...
  [/\ [/\ orthogonal calSnu 1%CF, '[beta, 1] = 1 & beta \in 'Z[irr G]],
      exists2 Gamma : 'CF(G), [/\ orthogonal calSnu Gamma, '[Gamma, 1] = 0
             & exists2 a, a \in Cint & beta = 1 - nu zeta + a *: sumSnu + Gamma]
    & e <= (h - 1) / 2%:R ->
            '[(nu zeta)^\rho] >= 1 - e / h /\ '[Gamma] <= e - 1
    & {in irr G, forall chi : 'CF(G), orthogonal calSnu chi ->
...
Hypothesis normedTI_A : forall i, normedTI (A i) G (L i).

Hypothesis card_coprime : forall i j, i != j -> coprime #|H i| #|H j|.
