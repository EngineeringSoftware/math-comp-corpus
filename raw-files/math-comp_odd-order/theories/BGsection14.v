...
Proposition Ptype_structure M K (Ms := M`_\sigma) (Kstar := 'C_Ms(K)) :
  ...
  [/\ exists2 U : {group gT},
            ...
            & [/\ abelian U, semiprime Ms K & semiregular U K], K \x Kstar = 'N_M(K)
           /\ {in 'E^1(K), forall X, 'N_M(X) = 'N_M(K)
           /\ {in 'M('N(X)), forall Mstar, X \subset Mstar`_\sigma}}, Kstar != 1 /\ {in 'E^1(Kstar), forall X, 'M('C(X)) = [set M]},
  [/\ {in ~: M, forall g, Kstar :&: M :^ g = 1}
         /\ {in M :\: 'N_M(K), forall g, K :&: K :^ g = 1}, {in \pi(Kstar), forall p S,
             p.-Sylow(M) S -> 'M(S) = [set M] /\ ~~ (S \subset Kstar)}
    & forall Y, \sigma(M).-group Y -> Y :&: Kstar != 1 -> Y \subset Ms]
    & M \in 'M_'P2 ->
...
Corollary pi_of_cent_sigma M x x' :
    ...
    x' \in ('C_M[x])^# -> \sigma(M)^'.-elt x' ->  \kappa(M).-elt x' /\ 'C[x] \subset M
  \/ [/\ \tau2(M).-elt x', \ell_\sigma(x') == 1%N & 'M('C[x']) = [set M]].
Proof.
...
Theorem FT_signalizer_context x (N := 'N[x]) (R := 'R[x]) :
  ...
  [/\ [/\ [transitive R, on 'M_\sigma[x] | 'JG],
        ...
        & Hall 'C[x] R]
   & #|'M_\sigma[x]| > 1 ->
  [/\ 'M('C[x]) = [set N], R :!=: 1, \tau2(N).-elt x, N \in 'M_'F :|: 'M_'P2
    & {in 'M_\sigma[x], forall M,
  [/\ R ><| 'C_(M :&: N)[x] = 'C[x], {subset \tau2(N) <= \sigma(M)}, {subset [predI \pi(M) & \sigma(N)] <= \beta(N)}
    & \sigma(N)^'.-Hall(N) (M :&: N)]}]].
Proof.
...
Theorem Ptype_embedding M K :
  ...
  [/\ {in 'E^1(K), forall X, 'M('C(X)) = [set Mstar]}, \kappa(Mstar).-Hall(Mstar) Kstar /\ \sigma(M).-Hall(Mstar) Kstar, 'C_(Mstar`_\sigma)(Kstar) = K /\ \kappa(M) =i \tau1(M), [/\ cyclic Z, M :&: Mstar = Z,
                {in K^#, forall x, 'C_M[x] = Z},
                {in Kstar^#, forall y, 'C_Mstar[y] = Z}
              & {in K^# & Kstar^#, forall x y, 'C[x * y] = Z}]
& [/\ [/\ normedTI Zhat G Z, {in ~: M, forall g, [disjoint Zhat & M :^ g]}
              & (#|G|%:R / 2%:R < #|class_support Zhat G|%:R :> rat)%R ], M \in 'M_'P2 /\ prime #|K| \/ Mstar \in 'M_'P2 /\ prime #|Kstar|, {in 'M_'P, forall H, gval H \in M :^: G :|: Mstar :^: G}
    & M^`(1) ><| K = M]].
Proof.
...
Corollary mFT_partition :
  ...
  [/\ 'M_'P == set0 :> {set {group gT}} -> partition Pcover G^#
...
Lemma primes_non_Fitting_Ftype M E q Q :
  ...
  [\/ q \in \tau2(Mstar) /\ 'M('C(Q)) = [set Mstar]
    | q \in \kappa(Mstar) /\ Mstar \in 'M_'P1 ].
Proof.
