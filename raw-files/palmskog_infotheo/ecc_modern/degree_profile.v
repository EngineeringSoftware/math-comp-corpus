...
Definition tree_dist_children k i (s : seq K) :=
...
Hypothesis Hpb : p \in border (nodes c).
...
rewrite (partition_big
           (fun x : {set port} => nth ord0 (enum 'I_U) #|x|) xpredT) //=.
...
Let bnext (c : comp_graph port) := head def_port (enum (border (nodes c))).
...
Definition remaining_ports k :=
  Num.max 0 ((#|port|%:R * lam `_ k) - (k.+1 * #| [node n $ k.+1 | n \in h] |)%:R).
