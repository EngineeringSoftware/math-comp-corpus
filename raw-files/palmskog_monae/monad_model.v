...
refine (@Monad_of_bind_ret m0
  (fun A B m f => fun s => let (a, s') := m s in f a s')
  (fun A a => fun s => (a, s))
   _ _ _).
by [].
...
(fun A (a : A) s => [fset (a : choice_of_Type A, s : choice_of_Type S)]) _ _ _.
Next Obligation.
...
((fun s => [fset (s : choice_of_Type S , s : choice_of_Type S)]) : (_monad S S))
((fun s => (fun _ => [fset (tt : choice_of_Type unit, s : choice_of_Type S)])) : S -> (_monad S unit))
_ _ _ _))).
Next Obligation.
