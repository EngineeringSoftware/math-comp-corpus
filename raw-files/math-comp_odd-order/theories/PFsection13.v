...
Lemma FTtypeP_facts :
  [/\ [/\ pred2 2 3 (FTtype S), q < p -> FTtype S == 2,
                [Frobenius U <*> W1 = U ><| W1] & abelian U], p.-abelem P /\ #|P| = p ^ q, u <= (p ^ q).-1 %/ p.-1, coherent calS S^# tau
...
Import ssrint.
Let lb_m_cases :
 [/\ (q >= 7)%N -> m > 8%:R / 10%:R, (q >= 5)%N -> m > 7%:R / 10%:R
   & q = 3 ->
...
Lemma FTtypeII_support_facts T L (Q := T`_\F) (H := L`_\F) :
  ...
  [/\ [Frobenius L with kernel H], U \subset H
...
Lemma FTtypeP_bridge_facts (V_S := class_support (cyclicTIset defW) S) :
  [/\ [/\ forall j, j != 0 -> beta_ j \in 'CF(S, 'A0(S))
              & forall j, j != 0 -> beta_ j \in 'CF(S, P^# :|: V_S)], forall j, j != 0 -> '[beta_ j] = (u.-1 %/ q + 2)%:R, [/\ forall j, j != 0 -> tau (beta_ j) - 1 + eta_ 0 j = Gamma,
                '[Gamma, 1] = 0 & cfReal Gamma], forall X Y : 'CF(G),
              Gamma = X + Y -> '[X, Y] = 0 ->
...
Lemma FTtypeI_bridge_facts :
  [/\ 'A~(L) :&: (class_support P G :|: class_support W G) = set0, orthogonal (map tau1 calL) (map sigma (irr W)), forall j, j != 0 -> '[tauL betaL, eta_ 0 j] = '[tauL betaL, eta01]
