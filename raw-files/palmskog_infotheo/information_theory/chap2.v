...
Lemma chain_rule : JointEntropy.h PQ = `H P + CondEntropy.h QP.
Proof.
rewrite /JointEntropy.h {1}/entropy.
transitivity (- (\rsum_(a in A) \rsum_(b in B)
    PQ (a, b) * log (P a * \Pr_QP [ [set b] | [set a] ]))).
...
transitivity (
  ...
  - (\rsum_(a in A) \rsum_(b in B) PQ (a, b) * log (\Pr_QP [ [set b] | [set a] ]))).
...
transitivity (- (\rsum_(a in A) \rsum_(b in B) PQ (a, b) * log (P a)) +
  \rsum_(a in A) \rsum_(b in B) PQ (a, b) * log (\Pr_PQ [ [set a] | [set b] ])).
...
Lemma miE3 : mi PQ = `H Q - CondEntropy.h QP.
Proof. by rewrite miE2 entropyB. Qed.

Lemma miE4 : mi PQ = `H P + `H Q - `H PQ.
Proof.
...
Lemma chain_rule_information : MutualInfo.mi PY = \rsum_(i < n.+1)
    if i == O :> nat then
...
Let Q : dist B := Bivar.snd (Bivar.fst PQR).
Let R := Bivar.snd PQR.
Lemma mi_bound : Bivar.fst PQR = P `x Q ->
