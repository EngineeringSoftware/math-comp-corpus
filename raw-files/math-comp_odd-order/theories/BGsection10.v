...
Theorem sigma_group_trans M p X :
  ...
  [/\ forall g, X \subset M -> X :^ g \subset M ->
            exists2 c, c \in 'C(X) & exists2 m, m \in M & g = c * m, [transitive 'C(X), on [set Mg in M :^: G | X \subset Mg] | 'Js ]
    & X \subset M -> 'C(X) * 'N_M(X) = 'N(X)].
Proof.
...
Corollary beta'_cent_Sylow p q X :
  ...
  [/\ exists2 P, p.-Sylow(M`_\sigma) (gval P) & X \subset 'C(P), p \in \alpha(M) -> 'C_M(X)%G \in 'U
    & q.-Sylow(M^`(1)) X ->
...
Proposition max_normed_2Elem_signaliser p q (A Q : {group gT}) :
  ...
  exists2 P : {group gT}, p.-Sylow(G) P /\ A \subset P
          & [/\ 'O_p^'('C(P)) * ('N(P) :&: 'N(Q)) = 'N(P), P \subset 'N(Q)^`(1)
              & q.-narrow Q -> P \subset 'C(Q)].
Proof.
...
Lemma basic_p2maxElem_structure p A P :
  ...
  [/\ Z0 \in 'E_p^1(A), exists Y : {group gT},
            [/\ cyclic Y, Z0 \subset Y
              & forall A0, A0 \in 'E_p^1(A) :\ Z0 -> A0 \x Y = 'C_P(A)]
    & [transitive 'N_P(A), on 'E_p^1(A) :\ Z0| 'JG]].
Proof.
