...
Lemma coherent_seqIndD_bound (A B C D : {group gT}) :
        [/\ A <| L, B <| L, C <| L & D <| L] -> [/\ A \proper K, B \subset D, D \subset C, C \subset K
          & D / B \subset 'Z(C / B)]%g -> coherent (S A) L^# tau -> \unless coherent (S B) L^# tau,
...
Theorem bounded_seqIndD_coherence M H H1 :
   [/\ M <| L, H <| L & H1 <| L] ->
   [/\ M \subset H1, H1 \subset H & H \subset K] -> nilpotent (H / M) -> coherent (S H1) L^# tau -> (#|H : H1| > 4 * #|L : K| ^ 2 + 1)%N ->
 coherent (S M) L^# tau.
Proof.
...
Definition odd_Frobenius_quotient M (H1 := K^`(1) <*> M) :=
  [/\ odd #|L|, [/\ M <| L, M \subset K & nilpotent (K / M)]
    & [Frobenius L / H1 with kernel K / H1] ]%g.

Lemma non_coherent_chief M (H1 := (K^`(1) <*> M)%G) :
     odd_Frobenius_quotient M -> \unless coherent (S M) L^# tau,
   [/\ chief_factor L H1 K /\ (#|K : H1| <= 4 * #|L : K| ^ 2 + 1)%N
     & exists2 p : nat, p.-group (K / M)%g /\ ~~ abelian (K / M)
     & ~~ (#|L : K| %| p - 1)].
Proof.
...
Theorem Sibley_coherence L H W1 : [/\ odd #|L|, nilpotent H & normedTI H^# G L] -> let calS := seqIndD H L H 1 in let tau := 'Ind[G, L] in [\/ [Frobenius L = H ><| W1]
         | exists2 W2 : {group gT}, prime #|W2| /\ W2 \subset H^`(1)%G
