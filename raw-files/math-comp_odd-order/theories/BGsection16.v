...
Definition of_typeF M U (H := M`_\F) :=
 [/\ [/\ H != 1, U :!=: 1 & H ><| U = M], exists U1 : {group gT}, is_typeF_inertia M U U1
   & exists U0 : {group gT}, is_typeF_complement M U U0].

Definition of_typeI M (H := M`_\F) U :=
    of_typeF M U
  /\ [\/ normedTI H^# G M, abelian H /\ 'r(H) = 2
       | {in \pi(H), forall p, exponent U %| p.-1}
...
Definition of_typeP defW :=
  [/\ [/\ cyclic W1, Hall M W1, W1 != 1 & M' ><| W1 = M], [/\ nilpotent U, U \subset M', W1 \subset 'N(U) & H ><| U = M'], [/\ ~~ cyclic H, M^`(2) \subset 'F(M), H * 'C_M(H) = 'F(M)
              & 'F(M) \subset M'], [/\ cyclic W2, W2 != 1, W2 \subset H, W2 \subset M^`(2)
              & {in W1^#, forall x, 'C_M'[x] = W2}]
    & normedTI (W :\: (W1 :|: W2)) G W].
...
Definition of_typeV defW :=
  [/\ of_typeP defW /\ U = 1
   & [\/ normedTI H^# G M, exists2 p, p \in \pi(H) & #|W1| %| p.-1 /\ cyclic 'O_p^'(H)
      | exists2 p, p \in \pi(H)
...
Theorem BGsummaryA :
 [/\ [/\ M`_\sigma <| M, \sigma(M).-Hall(M) M`_\sigma &
               \sigma(M).-Hall(G) M`_\sigma], \kappa(M).-Hall(M) K /\ cyclic K, [/\ U \in [complements to M`_\sigma <*> K in M],
               K \subset 'N(U),
               M`_\sigma <*> U <| M,
               U <| U <*> K
             & M`_\sigma * U * K = M], {in K^#, forall k, 'C_U[k] = 1}
  &
 [/\ Kstar != 1 /\ {in K^#, forall k, K \x Kstar = 'C_M[k]}, [/\ M`_\F != 1, M`_\F \subset M`_\sigma, M`_\sigma \subset M^`(1),
               M^`(1) \proper M & nilpotent (M^`(1) / M`_\F)], [/\ M^`(2) \subset 'F(M), M`_\F * 'C_M(M`_\F) = 'F(M)
             & K :!=: 1 -> 'F(M) \subset M^`(1)]
   & M`_\F != M`_\sigma ->
...
Theorem BGsummaryB :
 [/\ forall p S, p.-Sylow(U) S -> abelian S /\ 'r(S) <= 2, abelian <<U :&: 'A(M)>>, exists U0 : {group gT},
           [/\ U0 \subset U, exponent U0 = exponent U & [disjoint U0 & 'A(M)]]
  & (forall X, X \subset U -> X :!=: 1 -> 'C_(M`_\sigma)(X) != 1 ->
            'M('C(X)) = [set M])
  /\ ('A(M) != 'A1(M) -> normedTI ('A(M) :\: 'A1(M)) G M)].
Proof.
split.
...
Theorem BGsummaryC : K :!=: 1 ->
 [/\
  [/\ abelian U /\ ~~ ('N(U) \subset M), [/\ cyclic Kstar, Kstar != 1, Kstar \subset M`_\F & ~~ cyclic M`_\F]
    & M`_\sigma ><| U = M^`(1) /\ Kstar \subset M^`(2)],
  exists Mstar,
  [/\ [/\ Mstar \in 'M_'P, 'C_(Mstar`_\sigma)(Kstar) = K,
                \kappa(Mstar).-Hall(Mstar) Kstar
              & 'M_\sigma(K) = [set Mstar]], {in 'E^1(Kstar), forall X, 'M('C(X)) = [set M]}
         /\ {in 'E^1(K), forall Y, 'M('C(Y)) = [set Mstar]}, [/\ M :&: Mstar = Z, K \x Kstar = Z & cyclic Z]
    & (M \in 'M_'P2 \/ Mstar \in 'M_'P2)
         /\ {in 'M_'P, forall H, gval H \in M :^: G :|: Mstar :^: G}]
& [/\ normedTI Zhat G Z, let B := 'A0(M) :\: 'A(M) in
            B = class_support Zhat M /\ normedTI B G M, U :!=: 1 ->
...
Theorem BGsummaryD M : M \in 'M ->
 [/\ {in M`_\sigma &, forall x y, y \in x ^: G -> y \in x ^: M}, forall g (Ms := M`_\sigma), g \notin M ->
           Ms:&: M :^ g = Ms :&: Ms :^ g /\ cyclic (Ms :&: M :^ g), {in M`_\sigma^#, forall x,
            [/\ Hall 'C[x] 'C_M[x], 'R[x] ><| 'C_M[x] = 'C[x]
              & let MGx := [set Mg in M :^: G | x \in Mg] in
                [transitive 'R[x], on MGx | 'Js] /\ #|'R[x]| = #|MGx| ]}
   & {in M`_\sigma^#, forall x (N := 'N[x]), ~~ ('C[x] \subset M) ->
...
Theorem BGsummaryE :
  [/\ forall M, M \in 'M ->
            #|class_support M^~~ G| = (#|M`_\sigma|.-1 * #|G : M|)%N, {in \pi(G), forall p,
             exists2 M : {group gT}, M \in 'M & p \in \sigma(M)}
         /\ {in 'M &, forall M H,
             gval H \notin M :^: G -> [predI \sigma(M) & \sigma(H)] =i pred0}
    & let PG := [set class_support M^~~ G | M : {group gT} in 'M] in
  ...
  [/\ forall H x a, Hall G H -> nilpotent H -> x \in H -> x ^ a \in H ->
    ...
    \/ exists ST : {group gT} * {group gT}, let (S, T) := ST in
      [/\ S \in 'M /\ T \in 'M,
          ...
          let W := W1 <*> W2 in let V := W :\: (W1 :|: W2) in [/\ cyclic W, normedTI V G W & W1 :!=: 1 /\ W2 :!=: 1] /\ [/\ S^`(1) ><| W1 = S, T^`(1) ><| W2 = T & S :&: T = W], {in 'M, forall M, FTtype M != 1%N ->
            exists x, S :^ x = M \/ T :^ x = M}, FTtype S == 2 \/ FTtype T == 2
   & 1 < FTtype S <= 5 /\ 1 < FTtype T <= 5]].
Proof.
...
Theorem BGsummaryII M (X : {set gT}) :
 ...
 [/\       D \subset 'A1(M), {in X, forall x a, x ^ a \in X -> exists2 y, y \in M & x ^ a = x ^ y}
  &  {in D, forall x (L := 'N[x]),
 [/\ 'M('C[x]) = [set L], FTtype L \in pred2 1%N 2,
     [/\ L`_\F ><| (M :&: L) = L, {in X, forall y, coprime #|L`_\F| #|'C_M[y]| }, x \in 'A(L) :\: 'A1(L)
       & 'C_(L`_\F)[x] ><| 'C_M[x] = 'C[x]]
   & FTtype L == 2 ->
