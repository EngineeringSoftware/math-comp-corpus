...
Lemma kappa_structure M U K (Ms := M`_\sigma) :
  ...
  [/\ [/\ (Ms ><| U) ><| K = M, cyclic K & abelian (M^`(1) /  Ms)], K :!=: 1 -> Ms ><| U = M^`(1) /\ abelian U, forall X, X \subset U -> X :!=: 1 -> 'C_Ms(X) != 1 ->
            [/\ 'M('C(X)) = [set M], cyclic X & \tau2(M).-group X], abelian <<\bigcup_(x in Ms^#) 'C_U[x]>>
    & U :!=: 1 -> exists U0,
...
Theorem Fcore_structure M (Ms := M`_\sigma) :
 ...
 /\ (forall K D : {group gT},
     ...
     q^'.-Hall(M`_\sigma) D ->
    [/\ [/\ M \in 'M_'P1, Ms ><| K = M & Ms = M ^`(1)], [/\ prime p, prime q, q \in \pi(M`_\F) & q \in \beta(M)],
      [/\ q.-Sylow(M) Q, nilpotent D
        & Q0 <| M], [/\ minnormal Qbar (M / Q0), q.-abelem Qbar & #|Qbar| = (q ^ p)%N]
      & [/\ Ms^`(1) = M^`(2),
                  M^`(2) \subset 'F(M),
                  [/\ Q <*> 'C_M(Q) = 'F(M),
                      'C_M(Qbar | 'Q) = 'F(M)
...
Corollary Fitting_structure M (H := M`_\F) (Y := 'O_\sigma(M)^'('F(M))) :
    M \in 'M ->
  [/\ cyclic Y /\ \tau2(M).-group Y, [/\ M^`(2) \subset 'F(M),
                H \* 'C_M(H) = 'F(M)
              & 'F(M`_\sigma) \x Y = 'F(M)], H \subset M^`(1) /\ nilpotent (M^`(1) / H)
    & M \in 'M_'P -> 'F(M) \subset M^`(1)].
Proof.
...
Theorem nonTI_Fitting_structure M g (H := (M`_\F)%G) :
  let X := ('F(M) :&: 'F(M) :^ g)%G in
    M \in 'M -> g \notin M -> X :!=: 1 ->
  [/\ M \in 'M_'F :|: 'M_'P1 /\ H :=: M`_\sigma, X \subset H /\ cyclic X, M^`(1) \subset 'F(M) /\ M`_\sigma \x 'O_\sigma(M)^'('F(M)) = 'F(M), forall E E1 E2 E3, sigma_complement M E E1 E2 E3 ->
            [/\ E3 :=: 1, E2 <| E, E / E2 \isog E1 & cyclic (E / E2)]
    & [/\ M \in 'M_'F, abelian H & 'r(H) = 2]
         \/ let p := #|X| in [/\ prime p, ~~ abelian 'O_p(H), cyclic 'O_p^'(H)
          & {in \pi(H), forall q, exponent (M / H) %| q.-1}
         \/ [/\ #|'O_p(H)| = (p ^ 3)%N, M \in 'M_'P1 & #|M / H| %| p.+1]
  ]].
Proof.
...
Theorem nonFtype_signalizer_base M x :
   ...
   ~~ ('C[x] \subset M) -> 'N[x] \notin 'M_'F ->
 [/\ M \in 'M_'F, 'N[x] \in 'M_'P2
   & exists2 E : {group gT}, \sigma(M)^'.-Hall(M) E
