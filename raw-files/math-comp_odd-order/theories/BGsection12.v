...
Lemma sigma_compl_context M E E1 E2 E3 :
  ...
  [/\ E3 \subset E^`(1) /\ E3 <| E, E2 :==: 1 -> E1 :!=: 1, cyclic E1 /\ cyclic E3, E3 ><| (E2 ><| E1) = E /\ E3 ><| E2 ><| E1 = E
    & 'C_E3(E) = 1].
Proof.
...
Proposition p2Elem_mmax M p A :
      M \in 'M -> A \in 'E_p^2(M) -> 'C(A) \subset M
 /\ ([forall A0 in 'E_p^1(A), 'M('N(A0)) != [set M]] ->
...
Theorem tau2_context M p A (Ms := M`_\sigma) :
  ...
  [/\ forall P, p.-Sylow(M) P ->
                abelian P
             /\ (A \subset P -> 'Ohm_1(P) = A /\ ~~ ('N(P) \subset M)),  Ms <*> A <| M, 'C_Ms(A) = 1, forall Mstar, Mstar \in 'M(A) :\ M -> Ms :&: Mstar = 1
    & exists2 A1, A1 \in 'E_p^1(A) & 'C_Ms(A1) = 1].
Proof.
...
Corollary tau2_compl_context M E p A (Ms := M`_\sigma) :
  ...
  [/\ A <| E /\ 'E_p^1(E) = 'E_p^1(A), [/\ 'C(A) \subset E, 'N_M(A) = E & ~~ ('N(A) \subset M)], forall X, X \in 'E_p^1(E) -> 'C_Ms(X) != 1 -> 'M('C(X)) = [set M]
    & forall Mstar,
...
Corollary tau2_regular M E E1 E2 E3 p A (Ms := M`_\sigma) :
  ...
  [/\ semiregular Ms E3, semiregular Ms 'C_E1(A)
          & A \subset E2].
Proof.
...
Theorem nonabelian_tau2 M E p A P0 (Ms := M`_\sigma) (A0 := 'C_A(Ms)%G) :
 ...
 [/\ \tau2(M) =i (p : nat_pred), #|A0| = p /\ Ms \x A0 = 'F(M), forall X,
             X \in 'E_p^1(E) :\ A0 -> 'C_Ms(X) = 1 /\ ~~ ('C(X) \subset M)
   & exists2 E0 : {group gT}, A0 ><| E0 = E
   & forall x, x \in Ms^# -> {subset \pi('C_E0[x]) <= \tau1(M)}].
Proof.
...
Theorem abelian_tau2 M E E1 E2 E3 p A S (Ms := M`_\sigma) :
    ...
    p \in \tau2(M) -> A \in 'E_p^2(E) ->
    p.-Sylow(G) S -> A \subset S -> abelian S ->
  [/\ E2 <| E /\ abelian E2, \tau2(M).-Hall(G) E2, [/\       'N(A) = 'N(S),
                      'N(S) = 'N(E2),
                     'N(E2) = 'N(E3 <*> E2)
            & 'N(E3 <*> E2) = 'N('F(E))]
    & forall X, X \in 'E^1(E1) -> 'C_Ms(X) = 1 -> X \subset 'Z(E)].
Proof.
...
Corollary tau1_act_tau2 M E p A q Q (Ms := M`_\sigma) :
 ...
 [/\ [/\ A0 \in 'E_p^1(A), 'C_A(Ms) = A0 & A0 <| M], gval A0 \notin A1 :^: G
   & A1 \in 'E_p^1(A) /\ ~~ ('C(A1) \subset M)].
Proof.
...
Lemma primes_norm_tau2Elem M E p A Mstar :
    M \in 'M -> \sigma(M)^'.-Hall(M) E -> p \in \tau2(M) -> A \in 'E_p^2(E) ->
    Mstar \in 'M('N(A)) ->
 [/\ {subset \tau2(M) <= [predD \sigma(Mstar) & \beta(Mstar)]}, [predU \tau1(Mstar) & \tau2(Mstar)].-group (E / 'C_E(A))
   & forall q, q \in \pi(E / 'C_E(A)) -> q \in \pi('C_E(A)) ->
           [/\ q \in \tau2(Mstar),
...
Proposition sigma_subgroup_embedding M q X Mstar :
    ...
    Mstar \in 'M('N(X)) :\ M ->
 [/\ gval Mstar \notin M :^: G,
           forall S, q.-Sylow(M :&: Mstar) S -> X \subset S -> 'N(S) \subset M
    /\ q.-Sylow(Mstar) S
    & if q \in \sigma(Mstar) then
     [/\ Mstar`_\beta * (M :&: Mstar) = Mstar, {subset \tau1(Mstar) <= [predU \tau1(M) & \alpha(M)]}
       & M`_\beta = M`_\alpha /\ M`_\alpha != 1] else
     [/\ q \in \tau2(Mstar), {subset [predI \pi(M) & \sigma(Mstar)] <= \beta(Mstar)}
       & \sigma(Mstar)^'.-Hall(Mstar) (M :&: Mstar)]].
Proof.
