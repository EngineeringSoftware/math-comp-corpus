...
Definition primeTI_hypothesis (L K W W1 W2 : {set gT}) of W1 \x W2 = W :=
  [/\ [/\ K ><| W1 = L, W1 != 1, Hall L W1 & cyclic W1], [/\ W2 != 1, W2 \subset K & cyclic W2],
...
Theorem primeTIirr_spec :
 [/\ injective Imu2,
           ...
           forall i j, sigma (w_ i j) = delta_ j *: mu2_ i j, forall i j, {in W :\: W2, mu2_ i j =1 delta_ j *: w_ i j}
...
Definition prime_Dade_definition :=
  [/\ [/\ H <| L, W2 \subset H & H \subset K], [/\ A <| L, \bigcup_(h in H^#) 'C_K[h]^# \subset A & A \subset K^#]
    & A0 = A :|: class_support V L].
...
Theorem uniform_prTIred_coherent k (calT := uniform_prTIred_seq ptiWL k) :
    k != 0 -> [/\ pairwise_orthogonal calT, ~~ has cfReal calT, cfConjC_closed calT,
            'Z[calT, L^#] =i 'Z[calT, A]
          & exists2 psi, psi != 0 & psi \in 'Z[calT, A]] /\ (exists2 tau1 : {linear 'CF(L) -> 'CF(G)},
