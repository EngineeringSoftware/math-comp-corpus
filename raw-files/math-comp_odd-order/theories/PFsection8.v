...
Lemma typeF_context M U (H := M`_\F) :
    of_typeF M U ->
  [/\ forall U0, is_typeF_complement M U U0 -> #|U0| = exponent U, [Frobenius M = H ><| U] = Zgroup U
    & forall U1 (i : Iirr H),
...
Lemma typeP_context :
  [/\ H \x 'C_U(H) = 'F(M), U^`(1)%g \subset 'C(H) /\ (U :!=: 1%g -> ~~ (U \subset 'C(H))), normedTI (cyclicTIset defW) G W
...
Definition typeP_pair S T (W W1 W2 : {set gT}) (defW : W1 \x W2 = W) :=
 [/\      [/\ cyclicTI_hypothesis G defW, S \in 'M & T \in 'M], [/\ S^`(1) ><| W1 = S, T^`(1) ><| W2 = T & S :&: T = W]%g, (FTtype S == 2) || (FTtype T == 2), (1 < FTtype S <= 5 /\ 1 < FTtype T <= 5)%N
 & {in 'M, forall M, FTtype M != 1%N -> gval M \in S :^: G :|: T :^: G}].
...
Lemma FTtypeP_pair_cases : {in 'M, forall M, FTtype M == 1%N}
  \/ exists S, exists T, exists_typeP (fun _ => typeP_pair S T).
Proof.
...
Lemma FTtypeI_II_facts n (H := M`_\F) :
  ...
  if 0 < n <= 2 then
  [/\ forall p S, p.-Sylow(U) S -> abelian S /\ ('r(S) <= 2)%N, forall X, X != set0 -> X \subset U^# -> 'C_H(X) != 1%g ->
            'M('C(X)) = [set M]
    & let B := 'A(M) :\: 'A1(M) in B != set0 -> normedTI B G M
  ] else True.
Proof.
...
Lemma FTsupport_facts (X := 'A0(M)) (D := [set x in X | ~~('C[x] \subset M)]) :
  [/\ {in X &, forall x, {subset x ^: G <= x ^: M}}, D \subset 'A1(M) /\ {in D, forall x, 'M('C[x]) = [set 'N[x]]}
    & {in D, forall x (L := 'N[x]) (H := L`_\F),
        [/\ H ><| (M :&: L) = L /\ 'C_H[x] ><| 'C_M[x] = 'C[x], {in X, forall y, coprime #|H| #|'C_M[y]| }, x \in 'A(L) :\: 'A1(L)
          & 1 <= FTtype L <= 2
...
Theorem FT_Dade_support_partition :
  [/\
           \pi(G) =i [pred p | [exists M : {group gT} in 'M, p \in \pi(M`_\s)]], {in 'M &, forall M L,
                gval L \notin M :^: G -> coprime #|M`_\s| #|L`_\s| }, {in 'M, forall M, #|'A1~(M)| = (#|M`_\s|.-1 * #|G : M|)%N}
    & let PG := [set 'A1~(Mi) | Mi : {group gT} in 'M^G] in
...
Lemma FT_Dade_support_disjoint S T :
  ...
  [/\ FTsupports S T = ~~ [disjoint 'A1(S) & 'A(T)]
         /\ {in 'A1(S) :&: 'A(T), forall x,
               ~~ ('C[x] \subset S) /\ 'C[x] \subset T}, [exists x, FTsupports S (T :^ x)] = ~~ [disjoint 'A1~(S) & 'A~(T)]
    & [disjoint 'A1~(S) & 'A~(T)] \/  [disjoint 'A1~(T) & 'A~(S)]].
Proof.
