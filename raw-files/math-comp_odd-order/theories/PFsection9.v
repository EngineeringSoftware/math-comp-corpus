...
exists F.
  ...
  pose autF (f : {perm F}) := rmorphism f.
...
Lemma typeP_nonGalois_characters (not_Galois : ~~ typeP_Galois) :
  ...
  [/\ {in X_ H0, forall s, (a %| 'chi_s 1%g)%C}, size mu_ = p.-1 /\ {in mu_, forall mu_j, isIndHC mu_j}, exists t, isIndHC 'chi_t
    & let irr_qa := [pred zeta in irr M | zeta 1%g == (q * a)%:R] in
...
Lemma typeP_Galois_characters (is_Galois : typeP_Galois) :
  [/\ {in X_ H0, forall s, (u %| 'chi_s 1%g)%Cx},
            {in X_ H0C', forall s, 'chi_s 1%g = u%:R /\
             (exists2 xi : 'CF(HC), xi \is a linear_char & 'chi_s = 'Ind xi)}, size mu_ = p.-1 /\ {in mu_, forall mu_j, isIndHC mu_j}
    & all redM (S_ H0C') ->
            [/\ C :=: 1%g, u = (p ^ q).-1 %/ p.-1
...
have nCW1: W1 \subset 'N(C).
  by rewrite (subset_trans (joing_subr U W1)) ?normal_norm.
clear sS20 cohS2 sS12 has_irrHC lb_Sqa sU'C.
