...
Definition subcoherent S tau R :=
  [/\ [/\ {subset S <= character}, ~~ has cfReal S & cfConjC_closed S], {in 'Z[S, L^#], isometry tau, to 'Z[@irr gT G, G^#]}, pairwise_orthogonal S, {in S, forall xi : 'CF(L : {set gT}),
              [/\ {subset R xi <= 'Z[irr G]}, orthonormal (R xi)
                & tau (xi - xi^*%CF) = \sum_(alpha <- R xi) alpha]}
    & {in S &, forall xi phi : 'CF(L),
...
Lemma extend_coherent S1 xi1 chi :
    ...
    [/\ coherent S1 L^# tau, (xi1 1%g %| chi 1%g)%C
      & 2%:R * chi 1%g * xi1 1%g < \sum_(xi <- S1) xi 1%g ^+ 2 / '[xi]] ->
  coherent (chi :: chi^*%CF :: S1) L^# tau.
Proof.
