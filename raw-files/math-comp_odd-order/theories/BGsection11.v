...
Lemma exceptional_TIsigmaJ g q Q1 Q2 :
    ...
    q.-Sylow(M`_\sigma :^ g) Q2 -> A \subset 'N(Q2) -> Q1 :&: Q2 = 1
  /\ (forall X, X \in 'E_p^1(A) -> 'C_Q1(X) = 1 \/ 'C_Q2(X) = 1).
Proof.
...
Corollary exceptional_TI_MsigmaJ g :
  g \notin M -> A \subset M :^ g -> M`_\sigma :&: M :^ g = 1
 /\ M`_\sigma :&: 'C(A0 :^ g) = 1.
Proof.
...
Corollary exceptional_structure (Ms := M`_\sigma) :
  [/\ A :=: 'Ohm_1(P), 'C_Ms(A) = 1
    & exists2 A1, A1 \in 'E_p^1(A) & exists2 A2, A2 \in 'E_p^1(A) &
